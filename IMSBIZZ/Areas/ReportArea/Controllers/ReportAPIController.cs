...
namespace IMSBIZZ.Areas.ReportArea.Controllers
{
    ...
    public class ReportAPIController : ApiController
    {
        ...
        {
            var sales = _reportService.CustomerBalanceReport(@"SELECT s.InvoiceNumber,spd.BalanceAmnt,spd.GivenAmnt,spd.GrandTotal,pur.PartyName,pur.CreatedOn
                                                         FROM Sale AS s
        ...
        /// <returns></returns>
        [Route("CustomerBalanceReport"), HttpGet]
        public HttpResponseMessage CustomerInvoicesPreviosTransaction(int companyId, int branchId)
        {
            var sales = _reportService.CustomerInvoicesPreviosTransaction(@"SELECT s.PartyId,count(s.PartyId) AS SaleCount,p.PartyName,sum(spd.PaidAmnt) AS PaidAmnt,
        ...
        /// <param name="branchId">Branch Id Filter</param>
        /// <returns></returns>
        [Route("VendorBalanceReport"), HttpGet]
        public HttpResponseMessage VendorBalanceReport(int companyId, int branchId)
        {
            var purchase = _reportService.VendorBalanceReport(@"SELECT p.PurchaseId,pur.PartyName,pd.GivenAmnt,pd.BalanceAmnt,pd.GrandTotal,pur.CreatedOn,p.InvoiceNumber
                                                             FROM Purchase as p
        ...
        /// <returns></returns>
        [Route("CustomerBalanceReport"), HttpGet]
        public HttpResponseMessage VendorInvoicesPreviosTransaction(int companyId, int branchId)
        {
            var sales = _reportService.VendorInvoicesPreviosTransaction(@"SELECT p.PartyId,count(p.PartyId) AS PurchaseCount,par.PartyName,sum(ppd.PaidAmnt) AS PaidAmnt,
        ...
        {
            var sales = _reportService.GetVendorInvoicesDetail(@"WITH PurchaseAndPurchaseReturn
	                                                        (Id,Batch,Company,CompanyId,BranchId,CompanyAddress,Party,PartyAddress,StartDate,EndDate,InvoiceNumber,PoNo,Product,
	                                                        PurchaseRate,SaleRate,Quantity,TaxAmnt,DiscountAmnt,ProductAmount,TotalTax,TotalDiscount,TotalAmount,
	                                                        GrandTotal,GivenAmnt,BalanceAmnt,PaymentMode,Date,Type)
                                                        AS
                                                        (
                                                         (SELECT p.PurchaseId,b.BatchName,c.CompanyName,c.CompanyId,brnch.BranchId,c.CompanyAddress,pa.PartyName,pa.PartyAddress,f.StartDate,f.EndDate,
		                                                         p.InvoiceNumber,p.PoNo,ProductName,pd.PurchaseRate,pd.SaleRate,quantity,pd.TaxAmnt,isnull(pd.DiscountAmnt,0)
                                                                ,pd.amount,payd.TaxAmount,payd.DiscountAmount,payd.SubTotal,payd.GrandTotal,payd.GivenAmnt,payd.BalanceAmnt,
		                                                         pm.PaymentmodeName,p.CreatedOn,'Purchase'
                                                          FROM Purchase p 
                                                          Inner Join Purchasedetails pd on p.PurchaseId=pd.PurchaseId 
                                                          inner join Product on pd.ProductId=Product.ProductId 
                                                          inner join PaymentMode pm on p.[PaymentModeId]=pm.[PaymentModeId] 
                                                          inner join Batch b on b.BatchId=pd.BatchId
                                                          inner join Company c on c.CompanyId=p.CompanyId 
                                                          Inner join Branch brnch on brnch.CompanyId = c.CompanyId
                                                          inner join FinancialYear f on f.FinancialYearId=p.FinancialYearId
                                                          ...
                                                          UNion ALL 
                                                          (SELECT p.PurchaseId,b.BatchName,c.CompanyName,c.CompanyId,brnch.BranchId,c.CompanyAddress,pa.PartyName,pa.PartyAddress,f.StartDate,f.EndDate,
		                                                          p.InvoiceNumber,'',ProductName,pd.PurchaseRate,pd.saleRate,quantity,pd.TaxAmnt,
		                                                          isnull(pd.DiscountAmnt,0),pd.amount,payd.TaxAmount,payd.DiscountAmount,payd.SubTotal,
		                                                          payd.GrandTotal,payd.GivenAmnt,payd.BalanceAmnt,pm.PaymentmodeName,p.CreatedOn,'Return'
                                                          FROM Purchasereturn p  
                                                          inner join Purchasereturndetails pd on p.PurchaseReturnId=pd.PurchaseReturnId   
                                                          inner join Product on pd.ProductId=Product.ProductId 
                                                          inner join PaymentMode pm on p.[PaymentModeId]=pm.[PaymentModeId] 
                                                          inner join Batch b on b.BatchId=pd.BatchId 
                                                          inner join Company c on c.CompanyId=p.CompanyId
                                                          ...
                                                          SELECT * from PurchaseAndPurchaseReturn p   
                                                          where p.InvoiceNumber={0} 
	                                                           and p.CompanyId={1} 
	                                                           and p.BranchId={2} 
                                                          order by Date ASC", invoiceNumber, companyId, branchId).ToList();
            return Request.CreateResponse(HttpStatusCode.OK, sales);
        }
        ...
        {
            var purchase = _reportService.PurchaseMonthlyCount(@"SELECT COUNT(p.PurchaseId) AS purchas_count,SUM(ppd.GrandTotal) AS purchase_total 
                                                             FROM Purchase as p 
        ...
        {
            var purchase = _reportService.SaleMonthlyCount(@"SELECT COUNT(s.SaleId) AS sale_id,SUM(spd.GrandTotal) AS grandtotal
                                                             FROM Sale AS s 
                                                             INNER JOIN SalePaymentDetials AS spd on s.SaleId = spd.SaleId
        ...
        {
            var purchase = _reportService.PurchaseChartDetails(@"SELECT top 5 p.ProductName, count(pd.ProductId) as [ProductCount], sum(pd.amount) as [TotalPuchase] 
        ...
        {
            var purchase = _reportService.SaleChartDetails(@"SELECT TOP 5 p.ProductName, count(sd.ProductId) AS [ProductCount], sum(sd.amount) AS [TotalSaleAmount] 
                                                             FROM SaleDetails sd
        ...
        /// <param name="branchId">Branch Id Filter</param>
        /// <returns></returns>
        [Route("GodownProductsbyCompanyandbranch"), HttpGet]
        public HttpResponseMessage GodownProductsbyCompanyandbranch(int companyId, int branchId)
        {
            var productsInGodown = _reportService.GodownProductsbyCompanyandbranch(@"SELECT g.godownId,g.godownname,COUNT(p.ProductName)product_no,SUM(s.Quantity)quantity 
                                                                                     FROM Godown AS g
        ...
        /// </summary>
        /// <param name="branchId">companyId Id Filter</param>
        /// 
        /// <returns></returns>
        [Route("CompanyWiseVendorPaymentDetails"), HttpGet]
        public HttpResponseMessage CompanyWiseVendorPaymentDetails(int companyId, int branchId)
        {
            // create a new JSON object to write out
            var vendorPaymentDetails = _reportService.CompanyWiseVendorPaymentDetails(@"SELECT 
	                                                               ...
	                                                               FROM 
	                                                                ...
	                                                                WHERE 
		                                                              pur.CompanyId={0} AND pur.BranchId ={1}", companyId, branchId).ToList();



            return Request.CreateResponse(HttpStatusCode.OK, vendorPaymentDetails);
        }

        /// <summary>
        /// Get Company wise Customer payment detials 
        /// </summary>
        /// <param name="companyId">companyId Id Filter</param>
        /// <param name="branchId">branchId Id Filter</param>
        /// <returns></returns>
        [Route("CompanyWiseCustomerPaymentDetails"), HttpGet]
        public HttpResponseMessage CompanyWiseCustomerPaymentDetails(int companyId, int branchId)
        {
            // create a new JSON object to write out
            var vendorPaymentDetails = _reportService.ExecWithRowQuery(@"SELECT p.PartyName,s.InvoiceNumber,s.CreatedOn,spd.PaidAmnt,spd.BalanceAmnt,spd.GrandTotal
                                                                         FROM Sale AS s
                                                                         INNER JOIN SalePaymentDetials AS spd ON s.SaleId = spd.SaleId
                                                                         INNER JOIN Party AS p ON s.PartyId = p.PartyId
                                                                         WHERE s.CompanyId={0} AND s.BranchId={1} ", companyId, branchId).ToList();
            return Request.CreateResponse(HttpStatusCode.OK, vendorPaymentDetails);
        }
//        /// <summary>
//        /// Get Company wise Customer payment detials 
...
//        }
        ...
        /// <param name="branchId">branchId Id Filter</param>
        /// <returns></returns>
        [Route("ProductDetailsByCompanyandBranch"), HttpGet]
        public HttpResponseMessage ProductDetailsByCompanyandBranch(int companyId, int branchId)
        {
            // create a new JSON object to write out
            var vendorPaymentDetails = _reportService.ExecWithRowQuery(@"SELECT p.ProductName,p.HSNCode,p.ProductCode,s.Quantity,b.BatchName
                                                                         FROM Product as p 
                                                                         INNER JOIN Stock as s on p.ProductId=s.ProductId
                                                                         INNER JOIN Batch as b on b.BatchId=s.BatchId
                                                                         WHERE p.CompanyId={0} AND p.BranchId={1}
                                                                         ORDER BY p.CompanyId desc", companyId, branchId).ToList();
            return Request.CreateResponse(HttpStatusCode.OK, vendorPaymentDetails);
        }
    }
